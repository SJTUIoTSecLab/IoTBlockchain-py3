        ...
        if self.server.node_manager.tablsp.flagg==0:#当还能接受时（计算生成树之前）
             ...
             #判断什么时候能计算最小生成树
              k=len(self.server.node_manager.tablsp.lspgroup)
               #print "enter handle_RESP"
              if k>=self.server.node_manager.expectedClientNum-1:#k满足要求可以进入广播  tochange   #10个节点k>8
                   # 进行生成树的计算,在节点中储存这棵树
    ...
    def handle_sendrequest(self, payload):
        ...
        if payload["hash"] != -1:
            ...
            if self.server.node_manager.view == 0:
                ...
                cf.set('meta', 'primary_node_ip', str(payload["address"][0]))
                cf.set('meta', 'primary_node_port', str(payload["address"][1]))
...
class NodeManager(object):
    ...
    def __init__(self, ip, rootlist, port=0, genisus_node=False, is_committee_node=False, 
        ...
        self.numSeedNode = 0
        self.tablsp=LSP(self.address,self.node_id,rootlist)#每个节点储存的LSP表
    ...
    def minner(self):
        ...
        while True:
            ...
            if self.view % 5 == 0 and self.view >= 5 and self.failhashsend:
