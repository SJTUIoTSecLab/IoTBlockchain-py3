...
class ProcessMessages(socketserver.BaseRequestHandler):
        ...
        if not self.server.node_manager.finishflag:
            for b in self.server.node_manager.commitMessages:
                ...
                for b1 in self.server.node_manager.commitMessages:
                    if (b == b1):
                        count += 1
                if (count > old_div((2*self.server.node_manager.numSeedNode), 3)):
    ...
    def handle_sendreply(self, payload):
        ...
        if (self.server.node_manager.replyMessage > old_div((2*self.server.node_manager.numSeedNode), 3)) and (not self.server.node_manager.successflag):
...
class NodeManager(object):
    ...
    def __init__(self, ip, port=0, genisus_node=False, is_committee_node=False, expected_client_num=4, isServer=False):
        self.ip = ip
        self.port = port
